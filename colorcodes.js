...
function makeMorphy(element, rectangle) {
  const supportsAnimation =
    typeof CSS !== "undefined" && CSS.supports("animation: name");
  const supportsTransition =
    typeof CSS !== "undefined" && CSS.supports("transition: property");
      ...
      textParts.forEach((textPart, index) => {
        ...
        if (colorMatches[index]) {
          ...
          colorSpan.addEventListener("click", (e) => {
            const target_text = e.target.innerText;

            if (isColor(target_text)) {
